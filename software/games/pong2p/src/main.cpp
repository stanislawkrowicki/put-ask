...
#define BTN_LEFT_1 16
#define BTN_RIGHT_1 17
#define BTN_LEFT_2 18
#define BTN_RIGHT_2 19
...
void update_paddles() {
  if (digitalRead(BTN_LEFT_1) == LOW && p1X > 0) p1X -= 5;
  if (digitalRead(BTN_RIGHT_1) == LOW && p1X + PADDLE_WIDTH < SCREEN_WIDTH) p1X += 5;
  if (digitalRead(BTN_LEFT_2) == LOW && p2X > 0) p2X -= 5;
  if (digitalRead(BTN_RIGHT_2) == LOW && p2X + PADDLE_WIDTH < SCREEN_WIDTH) p2X += 5;
}
void reset_ball() {
